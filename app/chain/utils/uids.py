...
def create_balanced_groups(
    candidate_uids: List[int], target_size: int = 6, random_seed: int = None
) -> List[List[int]]:
    """
    ...
    if remainder == 0:
        ...
        if miners_used <= total_candidates:
            ...
            groups.append(
                shuffled_uids[remaining_start : remaining_start + (target_size - 1)]
            )
            groups.append(
                shuffled_uids[
                    remaining_start
                    + (target_size - 1) : remaining_start
                    + 2 * (target_size - 1)
                ]
            )
        else:
            ...
            if remaining_count > 0:
                groups.append(
                    shuffled_uids[remaining_start : remaining_start + remaining_count]
                )

    elif remainder == 2:
        # For remainder 2, create balanced groups
        miners_used = (
            (base_groups - 2) * target_size
            + 2 * (target_size - 1)
            + 2 * (target_size + 1)
        )
        ...
        if miners_used <= total_candidates:
            ...
            groups.append(
                shuffled_uids[last_2_start : last_2_start + (target_size - 1)]
            )
            groups.append(
                shuffled_uids[
                    last_2_start
                    + (target_size - 1) : last_2_start
                    + 2 * (target_size - 1)
                ]
            )
...
def tournament_group_miners(
    self,
    exclude: List[int] = None,
    specified_miners: List[int] = None,
    target_size: int = 6,
) -> List[List[int]]:
    """
