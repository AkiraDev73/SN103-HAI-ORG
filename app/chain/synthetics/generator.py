...
class SyntheticsGenerator:
    def __init__(
        self, client, synth_corpus_path="app/chain/synthetics/synth_corpus.json"
    ):
        self.client = client
    ...
    def _generate_user_response(self, dialog):
        ...
        prompt = user_generation_prompt.format(
            profile=profile, messages=messages_formatted
        )
        user_response = self.client.chat.completions.create(
            model="gpt-5-nano",
            messages=[
                {"role": "system", "content": "Follow instructions precisely."},
    ...
    def _get_synth_corpus(self):
        try:
            ...
            logging.warning(
                "Failed to load synthetic corpus from remote storage. Using local version instead."
            )
    ...
    async def _generate_new_dialog(self):
        ...
        dialog = {
            "dialog_id": datetime.now().isoformat(),
    ...
    def _combine_person_profile(self):
        ...
        fears = [
            random.choice(self.synth_corpus["fears"])
            for _ in range(random.randint(0, 2))
        ]
        issues = [
            random.choice(self.synth_corpus["issues"])
            for _ in range(random.randint(0, 1))
        ]
        traumas = [
            random.choice(self.synth_corpus["traumas"])
            for _ in range(random.randint(0, 1))
        ]
    ...
    def update_dialog(self, dialog_id, message):
        self.dialogs[dialog_id]["messages"].append(
            Message(role=Role.assistant, content=message)
        )
