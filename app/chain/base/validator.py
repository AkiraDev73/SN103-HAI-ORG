...
class BaseValidatorNeuron(BaseNeuron):
    ...
    def set_weights(self):
        ...
        if self.scores.sum() > 0:
            ...
            bt.logging.warning(
                "All scores are zero! Using small random weights to break symmetry."
            )
            # Use small random weights instead of uniform to break symmetry
            import numpy as np

            raw_weights = (np.random.random(len(self.scores)) * 0.1 + 0.01).astype(
                np.float32
            )
    ...
    def update_scores(self, rewards: torch.FloatTensor, uids: List[int]):
        ...
        for uid in uids:
            if uid >= len(self.scores) or uid < 0:
                bt.logging.error(
                    f"UPDATE_SCORES: INVALID UID: {uid} (scores length: {len(self.scores)})"
                )
                return
        ...
        bt.logging.debug(
            f"Alpha (moving_average_alpha): {self.config.neuron.moving_average_alpha}"
        )
        ...
        scattered_rewards: torch.FloatTensor = self.scores.scatter(
            0,
            torch.tensor(uids).clone().detach().to(self.device),
        ...
        if len(self.scores) > 0:
            ...
            for i, idx in enumerate(top_indices):
                ...
                bt.logging.debug(
                    f"UPDATE_SCORES:   {i+1}. UID {idx}: {self.scores[idx]:.6f} {marker}"
                )

            # Show bottom scores
            bottom_indices = torch.topk(
                self.scores, k=min(10, len(self.scores)), largest=False
            ).indices
