...
class Evaluator:
    ...
    def evaluate(self, query, responses, miner_uids=None):
        ...
        if len(responses) != len(miner_uids):
            bt.logging.error(
                f"Responses ({len(responses)}) and UIDs ({len(miner_uids)}) length mismatch"
            )
            return torch.zeros(len(responses))
        ...
        bt.logging.info(
            f"Tournament evaluation: {len(responses)} miners with full context"
        )
        ...
        tournament_result = self._evaluate_criterion_tournament(
            responses, selected_criteria, dialog_context, current_question
        )
    ...
    def _select_tournament_criteria(self, num_criteria=2):
        ...
        if num_criteria > 0 and remaining_criteria:
            ...
            available_criteria = [
                c for c in remaining_criteria if c["category"] not in used_categories
            ]

            if available_criteria:
                additional = random.sample(
                    available_criteria, min(num_criteria, len(available_criteria))
                )
                selected.extend(additional)
                used_categories.update([c["category"] for c in additional])

        bt.logging.info(
            f"Selected criteria: {[c['id'] for c in selected]} from categories: {list(used_categories)}"
        )
        return selected

    def _evaluate_criterion_tournament(
        self, responses, criteria, dialog_context, current_question
    ):
        """
        ...
        try:
            ...
            messages = literal_eval(
                self.prompt_facade.get_prompt("judge", **template_data)
            )
            ...
            return self._fallback_evaluation(
                responses, criteria, dialog_context, current_question
            )

    def _fallback_evaluation(
        self, responses, criteria, dialog_context=None, current_question=None
    ):
        ...
        if dialog_context:
            bt.logging.debug(
                f"Fallback evaluation with dialog context: {len(dialog_context)} messages"
            )
        if current_question:
            bt.logging.debug(
                f"Fallback evaluation with current question: {current_question}"
            )
        ...
        for i, (uid, response) in enumerate(zip(self.current_miner_uids, responses)):
            if response:
                ...
                for criterion_id, score in scores.items():
                    criteria_scores.append(Rubric(rubric_id=criterion_id, mark=score))

                miner_evaluations.append(
                    MinerEvaluation(miner_id=uid, criteria_scores=criteria_scores)
                )
