...
class EloManager:
    ...
    def set_validator_hotkey(self, validator_hotkey: str):
        ...
        bt.logging.info(
            f"✅ Validator hotkey set to {validator_hotkey} in ELO sync manager"
        )

    def submit_elo_rating(
        self,
        epoch: int,
        miner_hotkey: str,
        rating: int,
        validator_hotkey: str = None,
        miner_response: str = None,
    ) -> bool:
        try:
            ...
            if validator_hotkey is None:
                bt.logging.error(
                    "❌ No validator hotkey available for ELO rating submission"
                )
                return False
            ...
            if response.status_code == 200 and response.json().get("success"):
                bt.logging.debug(
                    f"ELO submitted: Epoch {epoch}, Miner {miner_hotkey[:10]}..., Rating {rating}"
                )
                return True
            else:
