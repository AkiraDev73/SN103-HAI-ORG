...
class Validator(BaseValidatorNeuron):
    def __init__(self):
        ...
        self.worker = Worker(
            worker_url=os.environ["WORKER_URL"], worker_port=os.environ["WORKER_PORT"]
        )
    ...
    def load_banned_miners(self):
        ...
        try:
            with open("banned_miners.json", "r") as f:
                ...
                bt.logging.info(
                    f"Loaded {len(self.banned_coldkeys)} banned coldkeys from banned_miners.json"
                )
    ...
    def get_banned_miner_uids(self):
        ...
        for uid in range(self.metagraph.n.item()):
            try:
                ...
                if neuron_info and neuron_info.coldkey in self.banned_coldkeys:
                    ...
                    bt.logging.debug(
                        f"Found banned miner: UID {uid} with coldkey {neuron_info.coldkey}"
                    )
            except Exception as e:
        ...
        if banned_uids:
            bt.logging.warning(
                f"Excluded {len(banned_uids)} banned miners from validation: UIDs {banned_uids}"
            )
    ...
    def get_previous_elo_scores(
        ...
        try:
            ...
            for uid in miner_uids:
                ...
                if response.status_code == 200:
                    ...
                    if result["found"]:
                        ...
                        bt.logging.debug(
                            f"Miner {uid} ({miner_hotkey}): Current ELO score {current_elo}, Epoch {current_epoch}"
                        )
                    else:
                        previous_scores[uid] = 1000
                        bt.logging.debug(
                            f"Miner {uid} ({miner_hotkey}): No previous score, using default 1000"
                        )
                else:
                    bt.logging.error(
            ...
            bt.logging.info(
                f"Retrieved previous ELO scores for {len(previous_scores)} miners, current epoch: {current_epoch}"
            )
    ...
    def submit_elo_ratings(
        self,
        epoch: int,
        miner_uids: List[int],
        rewards: List[float],
        previous_scores: Dict[int, float] = None,
        miner_responses: Dict[int, str] = None,
    ):
        """
        ...
        try:
            ...
            for uid, reward in zip(miner_uids, rewards):
                if reward > 0:  # Only submit positive ratings
                    ...
                    elo_bonus = int(
                        reward
                    )  # reward is now the ELO bonus (36, 30, 24, etc.)
                    ...
                    miner_hotkey = self.metagraph.hotkeys[uid]
                    validator_hotkey = (
                        self.metagraph.hotkeys[self.validator_uid]
                        if self.validator_uid < len(self.metagraph.hotkeys)
                        else None
                    )
                    ...
                    miner_response = (
                        miner_responses.get(uid) if miner_responses else None
                    )

                    if validator_hotkey:
                        success = self.elo_manager.submit_elo_rating(
                            epoch,
                            miner_hotkey,
                            accumulated_elo,
                            validator_hotkey,
                        ...
                        if success:
                            bt.logging.debug(
                                f"✅ ELO rating {accumulated_elo} (previous: {previous_elo} + bonus: +{elo_bonus}) submitted for miner {uid} ({miner_hotkey}), Response stored: {'Yes' if miner_response else 'No'}"
                            )
                        else:
                            bt.logging.warning(
                                f"⚠️ Failed to submit ELO rating for miner {uid} ({miner_hotkey})"
                            )
                    else:
                        bt.logging.error(
                            f"❌ Validator hotkey not available for UID {self.validator_uid}"
                        )
    ...
    def calculate_and_set_elo_weights(self, final_scores: Dict[str, Dict]):
        ...
        try:
            ...
            if max_elo == min_elo:
                ...
                bt.logging.info(
                    f"All miners have same ELO ({min_elo}), using equal weights"
                )
            else:
                ...
                for uid, elo_score in elo_data:
                    ...
                    bt.logging.info(
                        f"Miner {uid}: ELO {elo_score} -> Weight {normalized_weight:.4f}"
                    )
            ...
            bt.logging.info(
                f"✅ ELO weights calculated and pushed to blockchain: {len(elo_data)} miners"
            )
    ...
    async def forward(self):
        ...
        try:
            ...
            previous_scores, current_epoch = self.get_previous_elo_scores(
                all_miner_uids
            )
            bt.logging.info(
                f"Loaded previous ELO scores for {len(previous_scores)} miners, current epoch: {current_epoch}"
            )
            ...
            bt.logging.info(
                f"Querying {len(all_miner_uids)} miners with synthetic query..."
            )
            ...
            for _, (uid, response) in enumerate(zip(all_miner_uids, all_responses)):
                ...
                if response:
                    ...
                    if hasattr(response, "results") and response.results:
                        ...
                        bt.logging.debug(
                            f"Miner {uid}: Valid response in 'results' attribute"
                        )
                    # Check for response attribute (fallback)
                    elif hasattr(response, "response") and response.response:
                        response_content = response.response
                        bt.logging.debug(
                            f"Miner {uid}: Valid response in 'response' attribute"
                        )
            ...
            bt.logging.info(
                f"Working miners: {len(working_miner_uids)}/{len(all_miner_uids)}"
            )

            if len(working_miner_uids) < 2:
                bt.logging.warning(
                    "Not enough working miners for tournament evaluation (need at least 2)"
                )
                return
            ...
            for round_num in range(self.num_evaluation_rounds):
                bt.logging.info(
                    f"🔄 Starting evaluation round {round_num + 1}/{self.num_evaluation_rounds}"
                )
                ...
                round_groups = tournament_group_miners(
                    self, banned_uids, working_miner_uids, self.tournament_group_size
                )
                bt.logging.info(
                    f"Round {round_num + 1}: Created {len(round_groups)} new tournament groups"
                )

                # Process each tournament group in this round
                for group_idx, miner_uids in enumerate(round_groups):
                    bt.logging.info(
                        f"Round {round_num + 1}, Group {group_idx + 1}/{len(round_groups)}: {miner_uids}"
                    )
                    ...
                    bt.logging.info(
                        f"Round {round_num + 1}, Group {group_idx + 1}: {len(group_responses)}/{len(miner_uids)} working miners"
                    )

                    # Evaluate working miners in tournament for this round
                    if (
                        group_responses and len(group_responses) >= 2
                    ):  # Need at least 2 miners for tournament
                        try:
                            # Evaluate using tournament system (this runs multiple criteria internally)
                            working_rewards = self.evaluator.evaluate(
                                query, group_responses, group_working_uids
                            )

                            # Ensure working_rewards is a valid tensor/list
                            if working_rewards is None or len(working_rewards) != len(
                                group_working_uids
                            ):
                                bt.logging.error(
                                    f"Round {round_num + 1}, Group {group_idx + 1}: Invalid evaluation results"
                                )
                                continue
                            ...
                            for uid, reward in zip(group_working_uids, working_rewards):
                                ...
                                bt.logging.debug(
                                    f"Round {round_num + 1}, Miner {uid}: Score {reward:.4f}"
                                )

                        except Exception as e:
                            bt.logging.error(
                                f"Round {round_num + 1}, Group {group_idx + 1} failed: {e}"
                            )
                            continue
                    else:
                        bt.logging.warning(
                            f"Round {round_num + 1}, Group {group_idx + 1}: No working miners for evaluation"
                        )

                bt.logging.info(
                    f"🔄 Completed evaluation round {round_num + 1}/{self.num_evaluation_rounds}"
                )
            ...
            for uid, round_scores in miner_scores.items():
                if round_scores:  # Only include miners with valid scores
                    ...
                    bt.logging.info(
                        f"Miner {uid}: {len(round_scores)} rounds, average score: {avg_score:.2f}"
                    )

                    elo_bonus = int(
                        avg_score * 3.6
                    )  # This gives max 36 for perfect score (10 * 3.6)
                    ...
                    bt.logging.info(
                        f"Miner {uid}: Average score {avg_score:.2f} -> ELO bonus: +{elo_bonus}"
                    )
                else:
                    bt.logging.warning(
                        f"Miner {uid}: No scores available, skipping ELO calculation"
                    )

            if final_miner_uids and final_total_elo_bonuses:
                bt.logging.info(
                    f"Submitting total ELO bonuses for {len(final_miner_uids)} miners"
                )
                ...
                if new_epoch % 10 == 0:
                    bt.logging.info(
                        f"🎯 Epoch {new_epoch}: Applying soft forgetting to ELO bonuses before DB submission"
                    )
                    base_rating = 1000
                    gamma = 0.4

                    for i, (uid, bonus) in enumerate(
                        zip(final_miner_uids, final_total_elo_bonuses)
                    ):
                        previous_elo = previous_scores.get(uid, 1000)
                        current_elo = previous_elo + bonus
                        # Apply soft forgetting: R' = μ + γ * (R - μ)
                        forgotten_elo = base_rating + gamma * (
                            current_elo - base_rating
                        )
                        # Convert back to bonus
                        final_bonuses[i] = forgotten_elo - previous_elo
                        bt.logging.debug(
                            f"Miner {uid}: {current_elo} → {forgotten_elo} (bonus: {bonus} → {final_bonuses[i]})"
                        )

                # Submit to DB with forgotten bonuses
                self.submit_elo_ratings(
                    new_epoch,
                    final_miner_uids,
                    final_bonuses,
                    previous_scores,
                ...
                if final_scores:
                    ...
                    if validator_hotkey:
                        ...
                        bt.logging.info(
                            f"✅ Epoch {new_epoch} ELO weights calculated and set"
                        )
                    else:
                        bt.logging.error(
                            f"❌ Validator hotkey not available for UID {self.validator_uid}"
                ...
                bt.logging.warning(
                    "No ELO data collected - no miners were successfully evaluated"
                )
    ...
    def set_weights(self):
        ...
        try:
            ...
            if not hasattr(self, "elo_weights") or self.elo_weights is None:
                bt.logging.warning(
                    "⚠️ No ELO weights available, falling back to default weights"
                )
                super().set_weights()
                return

            bt.logging.info(
                "✅ Using ELO-based weights for blockchain weight assignment"
            )
            ...
            bt.logging.info(
                f"✅ ELO-based weights set successfully for {len(self.elo_weights)} miners"
            )
    ...
    def run(self):
        ...
        if self.validator_uid is None:
            try:
                ...
                bt.logging.info(
                    f"Validator hotkey set in ELO sync manager: {validator_hotkey}"
                )
