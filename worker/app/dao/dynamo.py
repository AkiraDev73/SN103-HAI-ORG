...
class DynamoDbDAO:
    ...
    def __init__(self):
        ...
        self.__table_questionnaire_summary = self.__dynamodb.Table(
            "questionnaire_summary"
        )
        self.__table_chat_memory = self.__dynamodb.Table(
            f"chat-memory-{os.getenv('ENV')}"
        )

    def create_initial_message(
        self, user_id: uuid.uuid4, chat_id: uuid.uuid4, message_info: MessageInfo
    ):
        item = {
            "user_id": str(user_id),
            "chat_id": str(chat_id),
            "messages": [message_info.to_dict()],
        }
    ...
    def update_questionnaire_summary(self, user_id: str, summary: str):
        ...
        try:
            response = self.__table_questionnaire_summary.update_item(
                ...
                ExpressionAttributeValues={":new_summary": summary},
                ReturnValues="UPDATED_NEW",
            )
            return response
        except ClientError as e:
            print(
                f"Failed to update questionnaire summary for user_id {user_id}: {e.response['Error']['Message']}"
            )
            raise e

    def persist_questionnaire_summary(self, user_id: str, summary: str):
        try:
            item = {"user_id": user_id, "summary": summary}
            ...
            print(
                f"Failed to persist questionnaire summary for user_id {user_id}: {e.response['Error']['Message']}"
            )
            raise e

    def add_chat_memory(self, user_id, chat_id, memory_text):
        item = {"user_id": str(user_id), "chat_id": str(chat_id), "memory": memory_text}
