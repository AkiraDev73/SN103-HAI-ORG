...
class PostgresDAO:
    ...
    def get_chat_info(self, user_id: str, chat_id) -> Chat:
        chat_info = (
            self.__db.query(Chat)
            .filter(Chat.id == chat_id and Chat.fk_user_id == user_id)
            .one_or_none()
        )
        if chat_info:
            return chat_info
    ...
    def delete_chat_info(self, chat_id: str, user_id: str) -> str:
        try:
            chat = (
                self.__db.query(Chat)
                .filter(and_(Chat.id == chat_id, Chat.fk_user_id == user_id))
                .first()
            )
