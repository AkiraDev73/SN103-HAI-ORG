...
def send_message(
    ...
    session_input: str = orchestrator_service.get_session_input(
        last_n_messages_str, request.user_input.text
    )
    next_q_type = orchestrator_service.classify_q_type(
        request.user_input.text, last_n_messages_str
    )

    is_issue = orchestrator_service.is_issue(request.user_input, last_n_messages)
    past_problem = llm_response_service.get_past_problem(
        next_q_type, last_n_messages_str
    )
    assistant_response_model = orchestrator_service.get_assistant_response_model(
        next_q_type
    )
    assistant_response_prompt = orchestrator_service.get_assistant_response_prompt(
        request.chat_llm_model, next_q_type, past_problem, last_n_messages_str
    )
    ...
    full_session_input = orchestrator_service.get_session_input(
        last_messages_str, request.user_input.text
    )
    assistant_response: str = llm_response_service.process_data(
        llm_response_model,  # pass temeperature
        full_session_input,
        temperature,
