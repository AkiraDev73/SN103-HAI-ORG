...
class RagService:
    def __init__(
        self, open_ai_client, pinecone_client, dynamodb_dao, llm_response_service
    ):
    ...
    def query_index_with_filter(
        self, query_text, user_id, top_k=2, score_threshold=0.3
    ) -> list[RagChat]:
        result = self.__rag_dao.query_index_with_filter(
            query_text, user_id, top_k, score_threshold
        )
    ...
    def process_chat_summary(self, chat, next_q_type):
        if next_q_type["question_flag"] == 35 and chat.session_name is not None:
            chat_summary = self.__llm_response_service.summarize_chat(
                chat.fk_user_id, str(chat.id)
            )
...
def get_rag_service(
    ...
    dynamodb_dao=Depends(get_dynamodb_dao),
    llm_response_service=Depends(get_llm_response_service),
):
    return RagService(
        open_ai_client, pinecone_client, dynamodb_dao, llm_response_service
    )
