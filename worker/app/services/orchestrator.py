...
class OrchestratorService:
    ...
    def classify_q_type(self, user_input: str, last_n_messages: str) -> dict:
        ...
        """
        next_q_type_claude: dict = self.model.process_data_gpt_json(
            chat_conversation, system_prompt=sys_prompt_orchestrator_claude
        )
    ...
    def is_issue(self, user_input, last_n_messages):
        ...
        return self.model.process_data_gpt_json(
            chat_conversation, system_prompt=sys_prompt_issue_classificator
        )["is_issue"]
    ...
    def get_assistant_response_prompt(
        ...
        if next_q_type["question_flag"] == 2:
            ...
            if chat_llm_model == "core" and past_problem:
                ...
                assistant_response_prompt = (
                    sys_q_type_22_relationship_past_experience.format(
                        generated_chat_summary=generated_chat_summary
                    )
                )
                ...
                assistant_response_prompt = self.get_current_sys_prompt(
                    next_q_type, chat_llm_model
                )
        ...
        return self.__embed_questionnaire_into_prompt(
            questions, assistant_response_prompt
        )

    def get_llm_model(self, session_input, next_q_type, is_issue):
        if is_issue or next_q_type["question_flag"] == 2:
            model = self.model.process_data_gpt_json(
                session_input, system_prompt=sys_prompt_model_selector
            )["selected"]
        else:
            model = "core"
...
def get_orchestrator_service(
    ...
    model=Depends(get_llm_response_service),
) -> OrchestratorService:
