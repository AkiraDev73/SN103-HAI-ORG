...
from app.config.common_sys_prompts import (
...
class ChatService:
    ...
    def create_initial_message(
        self, user_id: uuid.uuid4, chat_id: uuid.uuid4, user_input: SenderInput
    ) -> MessageInfo:
        message_info = MessageInfo(
            id=uuid.uuid4(), text=user_input.text, sender=user_input.sender, q_type="1"
        )
    ...
    def update_chat_name_and_llm_name(
        self, user_id: str, chat_id: str, session_name: str = None, model: str = "core"
    ):
        self.__postgres_dao.update_chat_name_and_llm_name(
            user_id, chat_id, session_name, model
        )
    ...
    def persist_message_to_dynamodb(
        self, user_id, chat_id, sender_input: SenderInput, q_type: int = None
    ) -> MessageInfo:
    ...
    def get_chat_last_messages(
        self, N: int, chat_id: str, user_id: str
    ) -> list[MessageInfo]:
        chat_messages_response: list[MessageInfo] = self.get_chat_messages(
            chat_id, user_id
        )
    ...
    def get_last_messages_as_string(
        self, N: int, user_id: str, chat_id: str
    ) -> list[MessageInfo]:
    ...
    def create_chat(
        self, user_id, user_name: str, recommendation_from_mood_journal: str
    ) -> tuple[Chat, MessageInfo]:
        chat_id = uuid.uuid4()
        ...
        sender_input = SenderInput(
            ...
            text=(
                DEFAULT_GREETING
                if recommendation_from_mood_journal is None
                else recommendation_from_mood_journal
            ),
        )

        chat = self.add_chat(
            Chat(id=chat_id, fk_user_id=user_id, started_at=datetime.datetime.now())
        )
    ...
    def process_chat_message(
        ...
        assistant_input: SenderInput,
        q_type,
    ) -> MessageInfo:

        _ = self.persist_message_to_dynamodb(
            user_id,
            send_message_request.chat_id,
            send_message_request.user_input,
            q_type,
        )
        assistant_message_info = self.persist_message_to_dynamodb(
            user_id, send_message_request.chat_id, assistant_input, q_type
        )
    ...
    def get_chat_name(self, chat: Chat, session_input, next_q_type, is_issue):
        if chat.session_name is None and is_issue:
            session_name = self.__orchestrator_service.get_session_name(
                session_input, sys_prompt_chat_name_creation
            )
            ...
            rethink_prompt_filled = sys_prompt_rethink_chat_name_if_needed.format(
                conversation_content=session_input, initial_chat_name=chat.session_name
            )
            session_name = self.__orchestrator_service.get_session_name(
                session_input, rethink_prompt_filled
            )
...
def get_chat_service(
    postgres_dao: PostgresDAO = Depends(get_postgres_dao),
