...
class LlmResponseService:
    ...
    def process_data(
        self, llm, user_input, temperature=0.2, model="", system_prompt=None
    ):
        if llm == "gpt":
            model = "gpt-5-nano" if model == "" else model
            return self.__try_process(
                self.process_data_gpt,
                self.process_data_claude,
                user_input,
                model,
                system_prompt,
                temperature,
    ...
    def process_data_gpt_json(
        self, user_input, model: str = "gpt-5-nano", system_prompt=None
    ) -> dict:
    ...
    def get_past_problem(self, next_q_type, last_n_messages_str):
        if next_q_type == 2:
            past_problem = self.process_data(
                llm="gpt",
                user_input=last_n_messages_str,
                system_prompt=sys_prompt_past_problem_classifier,
                model="gpt-5-nano",
            )
    ...
    def summarize_chat(self, user_id, chat_id):
        messages_info: list[MessageInfo] = self.__dynamodb_dao.get_chat_messages(
            user_id, chat_id
        )
...
def get_llm_response_service(
    open_ai_client: OpenAI = Depends(get_open_ai_client),
